(function() {
  ...
  function MyCoursesController($rootScope, $scope, $http) {
    ...
    function init() {
          if($rootScope.currentUser.profile.type == "client"){
            $http.get("./server/get-client-courses.php"+"?id="+$rootScope.currentUser.profile.clientId).then(res => {
            console.log(res.data.clCourses);
            $scope.courses = res.data.clCourses;
        })}
        if($rootScope.currentUser.profile.type == "trainer"){
          $http.get("./server/get-trainer-courses.php"+"?id="+$rootScope.currentUser.profile.trainerId).then(res => {
